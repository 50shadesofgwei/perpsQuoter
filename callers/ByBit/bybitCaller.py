...
from callers.ByBit.bybitCallerUtils import *
import json
...
class ByBitQuoter:
    ...
    def retry_with_backoff(self, func, *args):
        ...
        logger.error(f"ByBitCaller - Failed after {self.MAX_RETRIES} retries.")
        return None

    def get_quotes_for_all_symbols(self) -> dict:
        try:
            all_quotes = []
            all_symbols = BYBIT_TOKEN_LIST
            ...
            logger.error(f"ByBitCaller - An error occurred while processing symbols: {e}", exc_info=True)
            return None
    ...
    def get_all_quotes_for_symbol(
        ...
        try:
            ...
            logger.error(f"ByBitCaller - An error occurred while fetching all quotes for {symbol}: {e}", exc_info=True)
            return None
    ...
    def build_response_object(
        ...
        try:
            ...
            logger.error(f"ByBitCaller - An error occurred while fetching quote data for {symbol}: {e}", exc_info=True)
            return None
