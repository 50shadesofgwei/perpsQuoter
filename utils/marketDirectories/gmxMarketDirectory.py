...
class GMXMarketDirectory:
    ...
    def get_all_valid_symbols(cls) -> list:
        try:
            symbols_list = [data['market'] for data in cls._markets.values()]
            return symbols_list
    ...
    @classmethod
    def get_total_opening_fee(cls, symbol: str, is_long: bool, absolute_size_usd: float, open_interest: OpenInterest) -> float:
        try:
            skew_usd = 0.0
            skew_usd: float = cls.get_skew_usd_from_open_interest(
                symbol,
                open_interest
            )
            ...
            logger.error(f"GMXMarketDirectory - Failed to determine total opening fee for {symbol} with size {absolute_size_usd} and is_long = {is_long}. Error: {e}")
            return None
